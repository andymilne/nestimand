...
Converts `NA` values in `var` to a sentinel; numeric variables default to a sentinel of 0, and for factors the sentinel is placed first in the level order of the returned variable. The `where` predicate names the rows in which the variable is structurally undefined (e.g. `data$chord_type == "aug"`), and supplying it engages the checks that keep the two kinds of `NA` apart: an `NA` outside the declared rows is genuine missingness and is refused rather than converted, and a non-`NA` value inside them is inconsistent coding and is likewise refused. Omitting it converts every `NA` and warns that any which were genuine missing data are now coded as an existing condition.

...
The remedy is not to abandon these coefficients but to label them by what they estimate. Each is an exact linear combination of realized cell means, and which combination is fixed by the design rather than by a pivot, so a coefficient table can carry its own interpretation:

```
 term                     estimate  std.error  meaning
 (Intercept)               4.0844   0.2153     aug.none
 chord_typedim             0.1769   0.2625     -aug.none + dim.0
 chord_typemaj             1.0479   0.2625     -aug.none + maj.0
 chord_typedim:inversion1 -0.1064   0.2625     -dim.0 + dim.1
 training                  0.1157   0.0212     common slope
```

The `meaning` column is mechanical: it is read from the matrix relating cell means to identified effects, so it stays correct under any reordering of the factor levels, whereas the names do not. The reference condition follows the order declared in the data – the first level of the nested variable that is not the sentinel – rather than whichever column the pivot reaches last. A covariate entering additively is one slope common to every condition and is left as fitted; a covariate interacting with the conditions gives one slope per cell, which translates in the same way as the means, appearing as a reference-cell slope named for the covariate alone together with the differences from it. Under an ordinal family the reference condition has no free coefficient, its latent mean being absorbed into the thresholds, and that row is marked as such rather than left as a bare zero.
...
All four calls evaluate the model only at cells that exist, and all are stable under factor reordering. In `nestimand` these are `policy = "equal"` and `policy = "proportional"`, with `route = "cells"` for the reference-grid form and `route = "g_computation"` for the recipe below. On the balanced data every one returns maj − aug = 0.6779, equal to the hand-computed value; on the unbalanced variant the two weightings separate exactly as they should – equal 0.5347, proportional 0.6864 – with the two packages agreeing on each. (One caveat when covariates are in the model: the `emmeans` calls and the cell grid evaluate covariates at their means, whereas the observed-rows call carries each row's own covariate values. In a linear model with covariates balanced across conditions these coincide; the distinction becomes real in the situations the next route is built for.)
...
with a further term, `+ chord_type:inversion:training`, if the slope also varies by inversion. The covariate chain mirrors the mean structure, so the impossible covariate slopes are all-zero columns just as the impossible means are, and the `brms` declaration of Section 4.5 extends to them mechanically. Nothing about the recipes changes. On a variant of the demonstration data in which the training slope differs by chord type, both packages return the identical averaged contrast, stable under both factor orderings (maj − aug = 1.6468): `avg_predictions` on the restricted grid exactly as above, or `pairs(emmeans(m_mod, ~ chord_type, weights = "proportional"))`. The moderation itself is reported by evaluating the nominated-cell comparison of Section 4.4 at chosen covariate values:
...
> Major triads are rated between 0.25 and 1.05 above augmented triads, depending on the inversion in which major is realized; under equal weighting over inversions, 0.68.
...
and six such declarations silence the uninformative columns. This is what `nestimand` does when the chain parameterization is requested, deriving the declarations from the design matrix rather than assuming them, and the fit is correct. But silencing is not deletion, and four costs follow.
...
- [ ] Is the undefined level an explicit sentinel (`"none"`), never `NA`? (`apply_sentinel()` performs the conversion, refusing genuine missingness where told which rows are structural; `nesting_spec()` refuses `NA` outright. For hand-coding, list the sentinel first among the factor's levels, as Section 4 explains; the package finds it from the design and orders the chain form itself.)
- [ ] Is the model formula in chain form – `g1 + g1:g2 + …`, never crossed (`g1 * g2`)? Section 4 explains why. (`cell_formula()` gives either parameterization from a freely written formula; `nest_fit()` chooses between them and states the reason.)
- [ ] For a nested *continuous* variable: sentinel is 0, no free-standing main effect (`g + g:x`, never `g * x`), and check what value the reference grid assigned it before trusting any covariate-adjusted contrast (Section 5.2). (The same functions apply; a continuous variable is refused in any non-leaf position.)

After fitting:

- [ ] Compare `nobs(model)` with `nrow(data)` – did casewise deletion remove a condition?
- [ ] **Run the reorder test** (Section 3.3): permute the levels of *every* nested factor, refit, and recompute the reported estimates. Identical `logLik` is necessary but not the test – the fit is always unchanged; it is the *estimates* that must not move, and any that do were never identified. (`estimand()` runs the check by default, at the estimate level, and on the fixed-effects shadow model for mixed and Bayesian fits.)
- [ ] For `brms`: do the structurally impossible coefficients carry `constant(0)` priors – or deliberate regularization – derived from the actual design matrix rather than assumed (Section 4.5)? (`chain_priors()` derives the full block from the design matrix, distinguishing structural zeros from identification constraints.)
...
- [ ] Within-stratum contrasts: from `emmeans` (the chain formula itself declares the nesting), or from predictions restricted to realized cells – never from default `avg_comparisons` output containing sentinel rows. (`estimand(contrast = "within")` computes them.)
- [ ] Across-boundary contrasts: which weighting – equal, proportional, hierarchical, standardized from outside the design, or a nominated level – and is it stated in the methods? And over which rows was the model evaluated: the G-computation grid, or one row per condition at average covariates? For nominated-cell comparisons via the `emmeans` cell grid, remember `nesting = NULL` (Section 4.4). (`estimand()` makes the weighting an explicit argument, and the route over which the model is evaluated a second one, emitting the matching calls.)
...
## Appendix: the `nestimand` package

`nestimand` is an R package (`remotes::install_github("andymilne/nestimand")`). It fits the design in the realized-cell parameterization, where estimation is unconditionally well posed, and translates grids, labels, coefficients, draws, priors, and results back into the original variable space. Every function that computes also keeps the code that computed it, retrievable with `show_code()`, so an analysis can be read, saved, and adapted rather than taken on trust.

Five functions cover an ordinary analysis.
...
**`nesting_spec(data, formula, nests, fit, family, random)`**
Declares the analysis. The formula is the *desired* model, written freely – crossed terms welcome – and is transformed rather than fitted as given; random-effects terms may be written into it in the usual syntax. `nests` takes `"child %in% parent"` links, quoted or bare, and several independent families at once. `fit` selects the engine: `"lm"`, `"glm"`, `"lmer"`, `"glmer"`, `"clm"`, `"clmm"`, or `"brms"`. The function constructs the realized-cell factor and refuses what would otherwise pass silently: an `NA` in a nesting variable, a continuous variable in a non-leaf position, an ordered outcome sent to an engine that cannot model one, a name collision with the cell factor, and `||` on a factor, which does not give the diagonal covariance it appears to (Section 6.1). `apply_sentinel()` converts structurally undefined `NA` values to a sentinel, checking that they are structural when told which rows are.

**`nest_fit(spec, mode, random_structure, priors, dry_run, ...)`**
Fits the declared engine, assembling the call as text and evaluating it, so the returned model carries the code that made it and the declaration it came from; `estimand()` and `nest_summary()` can then be called on the model alone. Arguments in `...` reach the engine unaltered and appear in the saved code, which is how `chains`, `iter`, `seed`, and their kind are supplied; `dry_run = TRUE` returns the code without fitting, which matters when the fit is a sampling run. An `update` method carries the declaration across a refit. `random_structure` chooses how a declared random structure is translated – `"cells"` for an unstructured covariance over realized conditions, `"chain"` for the grouping-factor submodel, `"as_declared"` to pass it through – and only terms crossing the structural boundary are translated at all: a random intercept, a slope on the nesting variable, or a hand-written grouping structure comes back exactly as declared. A structure varying with the nested variable but not with its parent is refused rather than enlarged, with the alternatives named.

**`estimand(model, target, policy, route, weights, contrast, scale, ...)`**
Returns the contrast table in the original variable names. Two arguments, neither defaulted silently, determine what it means. `policy` weights the versions of a compound condition – `"equal"`, `"proportional"`, `"hierarchical"`, `"nominated"` with `at`, or a named vector supplied from outside the design – and `route` says over which rows the model is evaluated: `"g_computation"`, the default, crossing every observed row with every realized cell so that each condition is averaged over one covariate distribution, or `"cells"`, one row per condition with covariates at their means. `weights` is a third and separate choice, weighting the rows rather than the versions, which standardizes the estimand to a population other than the sample. `contrast = "within"` gives the per-stratum contrasts of Section 4.1, which cross no boundary and take no policy. A marginal contrast of a nested variable is pooled only over the strata in which that variable varies, since elsewhere it would leave its own levels and compare strata instead; the restriction is stated in the emitted code. Every across-boundary contrast carries the partial-identification bounds of Section 4.3 alongside the point estimate, and a reorder check runs by default – on the fixed-effects shadow model for mixed and Bayesian fits, since order instability is a fixed-effects phenomenon and refitting a large random structure can settle on a different optimum. For mixed fits the random effects are excluded explicitly, in the spelling the engine expects, since `re.form = NA` and `re_formula = NA` are each silently ignored by the other's engine.

**`nest_summary(model, space, random)`**
The coefficient table of Section 3.2, in either parameterization, with each row labelled by the conditions it equals. No refit is involved: the effect coefficients are an exact linear function of the cell means, with the standard errors a chain fit would give. `random = TRUE` adds the random-effects covariance, read out in full and labelled by conditions rather than by the fitted factor; `random_covariance()` gives it alone, in cell or effect space, and `random_heterogeneity()` the spread of a named effect across units – the quantity c′Σc of Section 6.4, built from the same policy that defines the corresponding fixed-effect contrast.

**`show_code(x)`**
The code that produced a result. It cannot drift from what was run, because the analysis is assembled as text and then evaluated: the code shown is the code that ran. `dry_run = TRUE` on `estimand()` or `nest_fit()` shows it beforehand instead.

Three further functions serve particular situations. `latent_estimand()` and `latent_draws()` compute a contrast as a linear functional of the coefficients, c′b, which is exact on any linear scale, costs one matrix product, and is the practical route for ordinal fits, where the prediction machinery returns per-category output and may refuse the comparison as too large; `latent_draws()` applies the same c draw by draw to a `brms` posterior. `nest_prior()` translates an elliptical prior between the space it is stated in and the space it is fitted in – µ ~ N(Am, ADA′), exact and needing no reparameterization – with `prior_audit()` and `prior_for_estimand()` reporting what a stated prior implies elsewhere and for a named contrast. `chain_priors()` derives the `constant(0)` declarations that the chain parameterization requires under `brms` (Section 4.5), taken from the design matrix rather than assumed, distinguishing structural zeros from identification constraints, and covering the fixed and random parts with the same columns.

**Which parameterization is used, and when.** Cells by default: nothing has to be derived, nothing held at zero, no coefficient named, and every frequentist engine is served on the same footing as `brms`. The chain form is selected when `emmeans` is the engine, since it works from the formula and needs the original factors as predictors; when a prior is stated in a form only effect coordinates can carry; or on request, for coefficients that read as effects. Fixed-effect estimands agree between the two to Monte Carlo error, and the emitted code names the parameterization and the reason for it.

**A note on sign conventions.** Contrasts in this document are written in the direction the engines produced when it was verified – maj − aug, and so on. `nestimand` fixes the direction itself rather than inheriting it, and reports the later declared level minus the earlier, so that a contrast reads as a departure from the reference condition: the same quantity appears there as maj − aug, with the opposite sign. Interactions follow the same rule in both factors.

`?nestimand` gives an index of every function, grouped by how often it is needed. The package's own test suite covers the numerical claims of this document.

*Every numerical claim in this document was verified by executing the code under R 4.3.3 with emmeans 1.10.0, marginaleffects 0.18.0, lme4 1.1-35.1, ordinal 2023.12-4, and brms 2.20.4 (August 2026). The double-bar and `diag()` behaviour of Section 6.1 was additionally verified under R 4.6.1 with lme4 2.0-6, and the `brms` results of Sections 4.5 and 6 under R 4.6.1 with brms and cmdstanr. Code blocks use the current `marginaleffects` hypothesis syntax (`hypothesis = ~ pairwise`); the verification environment's release used the equivalent string syntax of its day, and `nestimand` emits whichever the installed version accepts. Three conventions have changed across releases of that package – the `hypothesis` argument, the column carrying the contrast label, and the direction of the difference – and the last would flip reported signs, so the package fixes the direction itself rather than inheriting it. The core behaviours – the `NA` deletion, the aliasing, the reorder instability, and the Section 4 recipes – were additionally confirmed for ordinal models fitted with `clm()`, where `emmeans` and `marginaleffects` agree to machine precision whenever the estimand is defined identically (predicted category probabilities with covariates at their means). Package behaviour may change between versions: the reorder test should be rerun on the data and software at hand rather than these results being trusted.*

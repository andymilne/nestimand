  ...
  if (!length(cell_vars)) {
    ...
    if (!length(families))
      stop("nothing is nested here: no variable is undefined at any level of ",
           ...
           "for nestimand to translate. Fit it directly with lm(), lmer() or ",
           "brm(), which gives the same model. To use the estimand machinery ",
           "over the full crossing anyway, name the design variables in ",
           "`nests` without `%in%`",
